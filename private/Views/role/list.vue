        ...
        <div class="rightMain">
            ...
            <div id="list">
                <div class="search form-horizontal" style="padding:10px 20px 0 10px;overflow:auto;">
                    <button class="hd-button" v-on:click="add">添加角色</button>
                </div>
                <div class="row">
                    <div class="col-md-12">
                                ...
                                <table class="table table-striped table-bordered table-hover" id="mytable">
                                    ...
                                    <tbody id="resultTable">
                                        <tr v-for="data in datas" :key="data.ID">
                                            <td>{{data.ID}}</td>
                                            <td>{{data.RoleName}}</td>
                                            <td>{{data.Status ? '启用' : '停用'}}</td>
                                            <td>{{data.CreateTime | formatDate}}</td>
                                            <td>{{data.UpdatedTime | formatDate}}</td>
                                            <td align="center">
                                                <a v-on:click="mapping(data.ID)" class="btn btn-sm btn-outline filter-submit blue">
                                                    <i class="fa fa-edit"></i> 配  置
                                                </a>
                                                <a v-on:click="edit(data.ID)" class="btn btn-sm btn-outline filter-submit green">
    ...
    export default {
        ...
        methods: {
            pagechange: function (currentPage) {
                ...
                router.push({ name: 'RoleList', params: { size: self.display, pageno: self.current } });
                self.load();
            },
            load: function () {
                var self = this;
                httper.post('/role/list/' + self.display + '/' + self.current).then(function (response) {
            ...
            edit: function (id) {
                router.push({ name: 'RoleEdit', params: {id: id} });
            },
            mapping: function (id) {
                router.push({ name: 'Mapping', params: {id: id} });
            }
        }
    };
</script>